...
public class SubtractionStep {
    ...
    private final int subtractionEvaluation = number1 - number2;
    Actor actor;
    ...
    @Given("a {string} wants to subtraction two numbers")
    public void aWantsToSubtractionTwoNumbers(String name) {
    ...
    public void theApplicationPerformsTheSubtractionOfTheTwoNumbers() {
        HashMap<String, Object> header = headersXML.placeHeader(variables.getProperty("pathSubtract"));
        actor.attemptsTo(PostRequest.execute(header,variables.getProperty("pathCalculator"),body));
    }

    @Then("the application returns the result of the subtraction, responds with status {string}")
    public void theApplicationReturnsTheResultOfTheSubtractionRespondsWithStatus(String statusCode) {
        actor.should(
        ...
        try {
            ...
            assertThat(subtractionEvaluation).isEqualTo(parseInt(resultXml.resultXml(objectXMLToJSON)));
